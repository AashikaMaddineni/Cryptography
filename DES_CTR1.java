...
public class DES_CTR1{   
  ...
  byte[] performXOR(byte[] one, byte[] two) {
 ...
 byte[] permute (byte[] input, int[] mapping) {
 ...
 int getBitFromArray(byte[] array, int pos) {
  int value;
  int bytePos = pos / 8;
  int bitPos = pos % 8;  
  ...
  void setBitInArray(byte[] input, int pos, int value) {
  int bytePos = pos / 8;
  int bitPos = pos % 8;  
...
public byte[] CTRencrypt(byte[] message, byte[] key, byte[] nounce) {
  if (message.length < 8) {
  ...
  byte[] result=null;
  if (length == 8) {
   //System.out.println("nounce "+bytesToHex(nounce));
         byte[] temp = cryptText(nounce, key, "encrypt");
       //  System.out.println("temp "+bytesToHex(temp));
        result = XORBytes(temp, message);
    }
  return result;
}

public byte[] CTRdecrypt(byte[] message, byte[] key, byte[] nounce) {
	  if (message.length < 8) {
	   System.out.println("Message should be atleast 64 bits");
	   System.exit(1);
	  }
	  ...
	  if (length == 8) {
	   //System.out.println("nounce "+bytesToHex(nounce));
	         byte[] temp = cryptText(nounce, key, "decrypt");
	        // System.out.println("temp "+bytesToHex(temp));
	        result = XORBytes(temp, message);
	    }
	  return result;
...
public static byte[] XORBytes(byte[] in1, byte[] in2) {  
    ...
    { 
    	...
    	DES_CTR1 des = new DES_CTR1();
         System.out.println("Both Encryption and Decryption in same File\n");
           String Keyinput = "85741E3276A89617";
           byte[] key = des.hexStringToByteArray(Keyinput);          
    	   System.out.println("key "+bytesToHex(key));
    	   String nounceinput = "7192216071314143";
    	   byte[] nounce = des.hexStringToByteArray(nounceinput);
    	   System.out.println("nounce "+bytesToHex(nounce));
    	   ...
    	   System.out.println("key "+bytesToHex(key));    	  
    	   String counter1=des.bytesToHex(counter);
    	   Long counter2=(Long.valueOf(counter1)+i-1);
    	   //System.out.println(nounce2);
    	   String counter3 = Long.toString(counter2);
    	   counter=des.hexStringToByteArray(counter3);  
    	   //Here Counter increases by ctr, ctr+1, ctr+3, ctr+6..........
    	   System.out.println("counter "+bytesToHex(counter));
    	   //"HERE PLAINTEXT BLOCKS ARE RANDOMLY GENERATED"
    	   byte[] message = des.getmessage();
    	   System.out.println("Plaintext block "+(i-1)+": "+bytesToHex(message));
           byte[] cipher = des.CTRencrypt(message, key, nounce);       	   
    	   System.out.println("Encryption done! "+des.bytesToHex(cipher));
    	   byte[] demessage = des.CTRdecrypt(cipher, key, nounce);       	   
    	   System.out.println("Decryption done! "+des.bytesToHex(demessage));
    	  }
